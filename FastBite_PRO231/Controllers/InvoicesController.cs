...
public class InvoicesController : Controller
{
    ...
    {
        TempData["Error"] = "Bạn không có quyền truy cập chức năng hóa đơn.";
    ...
    private async Task<int?> GetCurrentEmployeeIdAsync()
    {
        ...
        return await _context.Employees
            .Where(employee => employee.UserId == userId.Value)
        ...
        string paymentMethod = "Cash")
    {
        ...
        else if (!selectedEmployeeId.HasValue && order.EmployeeId.HasValue)
        {
        ...
        var employees = await _context.Employees
            ...
            .Where(employee => employee.Status == OrderStatusConstants.StaffWorking)
            .OrderBy(employee => employee.User.FullName)
            .Select(employee => new InvoiceEmployeeOptionViewModel
            {
        ...
        var items = order.OrderDetails
            .Select(detail => new InvoiceLineViewModel
            {
                ProductId = detail.ProductId,
                ProductName = detail.Product == null ? "Sản phẩm" : detail.Product.ProductName,
    ...
    public async Task<IActionResult> Index(string? search, string? status)
    {
        ...
        if (status == "active")
        {
            query = query.Where(invoice => invoice.Status);
        }
        ...
        if (!string.IsNullOrWhiteSpace(search))
        {
            ...
            {
                query = query.Where(invoice =>
                    invoice.Order.Customer.User.FullName.Contains(search) ||
                    invoice.Order.Customer.User.UserName.Contains(search) ||
                    invoice.Order.Customer.User.Phone.Contains(search) ||
                    invoice.Employee.User.FullName.Contains(search));
            }
        }
        ...
        var model = new InvoiceManagementIndexViewModel
        {
            Search = search,
            StatusFilter = status,

            TotalInvoices = await _context.Invoices.CountAsync(),
            ActiveInvoices = await _context.Invoices.CountAsync(invoice => invoice.Status),
            CancelledInvoices = await _context.Invoices.CountAsync(invoice => !invoice.Status),
            ...
            Invoices = invoiceEntities
                .Select(invoice => new InvoiceManagementListItemViewModel
                {
        ...
        return View("~/Views/Admin/Invoice/Index.cshtml", model);
    }
    ...
    public async Task<IActionResult> Details(int id)
    {
        ...
        return View("~/Views/Admin/Invoice/Details.cshtml", model);
    }
    ...
    public async Task<IActionResult> Create(int orderId)
    {
        ...
        var model = await BuildCreateViewModelAsync(orderId);
        ...
        return View("~/Views/Admin/Invoice/Create.cshtml", model);
    }
    ...
    public async Task<IActionResult> Create(InvoiceCreateViewModel model)
    {
        ...
        if (!OrderStatusConstants.ValidInvoicePaymentMethods.Contains(
                model.PaymentMethod, StringComparer.OrdinalIgnoreCase))
        {
        ...
        {
            ModelState.AddModelError(nameof(model.EmployeeId), "Vui lòng chọn nhân viên lập hóa đơn.");
        }

        var order = await _context.Orders
            .Include(item => item.OrderDetails)
            .Include(item => item.Invoices)
            .FirstOrDefaultAsync(item => item.OrderId == model.OrderId);

        if (order == null)
        {
            ModelState.AddModelError(nameof(model.OrderId), "Đơn hàng không tồn tại.");
        }
        else
        {
            if (!IsCompletedStatus(order.Status))
            {
                ModelState.AddModelError(nameof(model.OrderId), "Đơn hàng chưa hoàn thành.");
            }

            if (order.Invoices.Any())
            {
                ModelState.AddModelError(nameof(model.OrderId), "Đơn hàng này đã có hóa đơn.");
            }

            if (order.OrderDetails.Count == 0)
            {
                ModelState.AddModelError(nameof(model.OrderId), "Đơn hàng không có sản phẩm.");
            }
        }
        ...
        if (model.EmployeeId.HasValue)
        {
            var employeeExists = await _context.Employees
                .AnyAsync(employee =>
                    employee.EmployeeId == model.EmployeeId.Value &&
                    employee.Status == OrderStatusConstants.StaffWorking);

            if (!employeeExists)
            {
                ModelState.AddModelError(nameof(model.EmployeeId), "Nhân viên không tồn tại hoặc đã nghỉ việc.");
            }
        }

        if (!ModelState.IsValid || order == null || !model.EmployeeId.HasValue)
        {
            var reloadModel = await BuildCreateViewModelAsync(model.OrderId, model.EmployeeId, model.PaymentMethod);
            ...
            return View("~/Views/Admin/Invoice/Create.cshtml", reloadModel);
        }
        ...
        try
        {
            ...
            TempData["Success"] = $"Đã tạo hóa đơn #{invoice.InvoiceId} thành công.";
    ...
    // HỦY HÓA ĐƠN — không xóa dữ liệu, chỉ chuyển Status = false.
    [HttpPost]
    [ValidateAntiForgeryToken]

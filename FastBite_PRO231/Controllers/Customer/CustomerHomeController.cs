...
public class CustomerHomeController : Controller
{
    ...
    public CustomerHomeController(FastBiteDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<IActionResult> Index()
    {
        ...
        if (!string.Equals(customer.User.Status, OrderStatusConstants.AccountActive, StringComparison.OrdinalIgnoreCase))
        {
            HttpContext.Session.Clear();

            TempData["Error"] = "Tài khoản khách hàng đã bị ngừng hoạt động.";
            return RedirectToAction("Login", "Login");
        }
        ...
        var totalSpent = await _context.Invoices
            .AsNoTracking()
            .Where(invoice => invoice.Status && invoice.Order.CustomerId == customer.CustomerId)
            .SumAsync(invoice => (decimal?)invoice.TotalAmount) ?? 0m;

        var promotionEntities = await _context.Promotions
            .AsNoTracking()
            .Include(promotion => promotion.PromotionDetails)
                .ThenInclude(detail => detail.Product)
            .Where(promotion => promotion.Status == OrderStatusConstants.PromotionActive)   
        ...
        var model = new CustomerDashboardViewModel
        {
            ...
            CartItemCount = customer.Cart?.CartItems.Sum(item => item.Quantity) ?? 0,
            ...
            ProcessingOrders = orders.Count(order =>
            ...
            RecentOrders = orders
                .Take(6)
                .Select(order => new CustomerDashboardOrderViewModel
                {
            ...
            Promotions = promotionEntities
                .Select(promotion =>
                {
                    ...
                    return new CustomerDashboardPromotionViewModel
                    {
        ...
        return View("~/Views/CustomerHome/Index.cshtml", model);
    }
}
...
public class CustomersController : Controller
{
    ...
    private IActionResult RedirectUnauthorized()
    {
        ...
        TempData["Error"] = "Chỉ tài khoản Admin mới được quản lý khách hàng.";
    ...
    public async Task<IActionResult> Index(string? search, string? status)
    {
        ...
        // Đã bỏ dòng c.User.FullName.Contains(search) bị lặp lại (lỗi gõ thừa).
        ...
        if (status == "active")
        {
            query = query.Where(c => c.User.Status == OrderStatusConstants.AccountActive);
        }
        ...
        var spending = await _context.Invoices
            .Where(i => i.Status && ids.Contains(i.Order.CustomerId))
            .GroupBy(i => i.Order.CustomerId)
            .Select(g => new { CustomerId = g.Key, Total = g.Sum(x => x.TotalAmount) })
            .ToListAsync();

        var model = new CustomerManagementIndexViewModel
        {
            Search = search,
            StatusFilter = status,
            ...
            ActiveCustomers = await _context.Customers.CountAsync(c =>
                c.User.Status == OrderStatusConstants.AccountActive),

            InactiveCustomers = await _context.Customers.CountAsync(c =>
                c.User.Status != OrderStatusConstants.AccountActive),

            CustomersWithOrders = await _context.Orders.Select(o => o.CustomerId).Distinct().CountAsync(),

            TotalPoints = await _context.Customers.SumAsync(c => (int?)c.Point) ?? 0,

            Customers = customers.Select(c =>
            {
                var order = orders.FirstOrDefault(x => x.CustomerId == c.CustomerId);
                var money = spending.FirstOrDefault(x => x.CustomerId == c.CustomerId);
        ...
        return View("~/Views/Admin/Customers/Index.cshtml", model);
    }
    ...
    public async Task<IActionResult> Details(int id)
    {
        ...
        var totalSpent = await _context.Invoices
            .AsNoTracking()
            .Where(i => i.Status && i.Order.CustomerId == id)
        ...
        var model = new CustomerManagementDetailsViewModel
        {
            ...
            CompletedOrders = orderEntities.Count(o => IsCompletedStatus(o.Status)),
            CancelledOrders = orderEntities.Count(o => IsCancelledStatus(o.Status)),
            TotalSpent = totalSpent,

            RecentOrders = orderEntities
                .Take(10)
                .Select(o => new CustomerOrderSummaryViewModel
                {
                    OrderId = o.OrderId,
        ...
        return View("~/Views/Admin/Customers/Details.cshtml", model);
    }
    ...
    public async Task<IActionResult> Edit(int id, CustomerManagementEditViewModel model)
    {
        if (!IsAdmin())
        {
            return RedirectUnauthorized();
        ...
        var duplicateUser = await _context.Users.AnyAsync(u =>
            u.UserName == model.UserName && u.UserId != customer.UserId);
        ...
        var duplicateEmail = await _context.Users.AnyAsync(u =>
            u.Email == model.Email && u.UserId != customer.UserId);
        ...
        var duplicatePhone = await _context.Users.AnyAsync(u =>
            u.Phone == model.Phone && u.UserId != customer.UserId);
        ...
        customer.User.Phone = model.Phone;
        customer.User.Status = model.Status;
    ...
    public async Task<IActionResult> ToggleStatus(int id)
    {
        ...
        customer.User.Status = customer.User.Status == OrderStatusConstants.AccountActive
            ? OrderStatusConstants.AccountInactive
            : OrderStatusConstants.AccountActive;

        await _context.SaveChangesAsync();

        TempData["Success"] = customer.User.Status == OrderStatusConstants.AccountActive
